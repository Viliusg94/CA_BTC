# SUKURTI FAILĄ: d:\CA_BTC\src\analysis\indicators_analysis.py
import pandas as pd
import numpy as np
...
def load_data_with_indicators():
    """
    Įkelia duomenis su techniniais indikatoriais iš duomenų bazės arba iš CSV failo
    ...
    # Pirmiausia patikriname, ar yra apdorotų duomenų CSV failas
    csv_path = "data/processed/btc_features.csv"
    if os.path.exists(csv_path):
        try:
            data = pd.read_csv(csv_path, index_col=0, parse_dates=True)
            print(f"Duomenys įkelti iš CSV failo: {csv_path}")
            return data
        except Exception as e:
            print(f"Klaida įkeliant duomenis iš CSV: {e}")
    
    # Jei nėra CSV failo, bandome gauti duomenis iš duomenų bazės
    ...
    try:
        ...
        print(f"Duomenys įkelti iš duomenų bazės: {len(price_data)} eilutės")
        return price_data
