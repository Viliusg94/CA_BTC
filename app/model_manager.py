...
class ModelManager:
    ...
    def _save_model(self, model, model_type, scaler):
        """
        Išsaugo modelį ir jo metrikas
        
        Args:
            model: TensorFlow modelis
            model_type (str): Modelio tipas
            scaler: Duomenų skalavimo objektas
        ...
        try:
            ...
            # Kelias iki modelio failo
            model_path = os.path.join(self.models_dir, model_type, f"{model_id}.h5")
            
            # Išsaugome modelį
            model.save(model_path)
            
            # Išsaugome scaler
            scaler_path = os.path.join(self.models_dir, model_type, f"{model_id}_scaler.joblib")
            ...
            self.logger.error(f"Klaida išsaugant modelį: {str(e)}")
            self.logger.error(traceback.format_exc())
            raise

    def get_predictions(self):
        """
        Gauna prognozes iš visų aktyvių modelių
        ...
        try:
            ...
            # KLAIDOS SPRENDIMAS: Užtikriname, kad prognozės turi tinkamą formatą
        
            # Patikrinkime, ar turime modelius ir jie aktyvūs
            for model_type in self.model_types:
                if model_type in self.statuses and self.statuses[model_type]['status'] == 'Aktyvus':
                    try:
                        # Gauname prognozes iš modelio
                        model_id = self.statuses[model_type].get('active_model_id')
                        if not model_id:
                            continue
                    
                        # Pabandykime gauti prognozes
                        model_predictions = self._predict_with_model(model_type, model_id)
                    
                        # Sukuriame žodyną su reikiamais laukais
                        prediction_item = {
                            'model': model_type,
                            'values': model_predictions.get('values', []),
                            'dates': model_predictions.get('dates', [])
                        }
                    
                        predictions.append(prediction_item)
                    except Exception as model_error:
                        self.logger.error(f"Klaida gaunant prognozes iš modelio {model_type}: {str(model_error)}")
        
            # Jei neturime prognozių, pridedame bent vieną fiksuotą
            if not predictions:
                from datetime import datetime, timedelta
                import random
            ...
            self.logger.error(f"Klaida gaunant prognozes: {str(e)}")
            return []
        
    # Pridėkime naują metodą modelio prognozėms gauti
    def _predict_with_model(self, model_type, model_id):
        """
        Gauna prognozes iš konkretaus modelio
        ...
        Returns:
            dict: Prognozių žodynas su dates ir values raktais
        """
        try:
            # Čia eitų tikra prognozė su modeliu
            # Šiuo atveju grąžiname imituotus duomenis
            ...
            for i in range(7):
                ...
                # Generuojame prognozuojamą kainą, pritaikytą modelio tipui
                if model_type == 'lstm':
                    variation = random.uniform(-500, 1500)
                    trend = i * 150
                elif model_type == 'gru':
                    variation = random.uniform(-800, 1200)
                    trend = i * 120
                elif model_type == 'transformer':
                    variation = random.uniform(-300, 1700)
                    trend = i * 180
                else:
                    variation = random.uniform(-1000, 1000)
                    trend = i * 100
            ...
            self.logger.error(f"Klaida prognozuojant su modeliu {model_type}: {str(e)}")
            return {'dates': [], 'values': []}
