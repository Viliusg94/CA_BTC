...
def get_positional_encoding(seq_len, d_model):
    """Sukuria pozicijos kodavimą Transformer modeliui"""
    pos = np.arange(seq_len)[:, np.newaxis]
    i = np.arange(d_model)[np.newaxis, :]
    angle_rates = 1 / np.power(10000, (2 * (i // 2)) / np.float32(d_model))
    angle_rads = pos * angle_rates
    
    # Taikome sin ir cos funkcijas
...
class TimeSeriesTransformer(tf.keras.Model):
    """Transformer modelis laiko eilučių prognozavimui"""
...
def create_transformer_model(input_shape, d_model=64, num_heads=4, ff_dim=128, dropout=0.1):
    """Funkcija Transformer modelio sukūrimui"""
